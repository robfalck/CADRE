...
from CADRE.EOMs.perturbations.total_perturbation import TotalPerturbation
from CADRE.EOMs.perturbations.spherical_harmonics_comp import SphericalHarmonicsComp
# from CADRE.EOMs.perturbations.secondary_body.secondary_body_comp import SecondaryBodyEffect
from CADRE.frame_conversions.r_comp import RComp
...
class Perturbations(om.Group):
    ...
    def setup(self):
        ...
        self.add_subsystem('r_comp', RComp(num_nodes=nn, cb=cb),
                           promotes_inputs=['p', 'f', 'g', 'L'], promotes_outputs=['r'])

        self.add_subsystem('spherical_harmonics', subsys=SphericalHarmonicsComp(num_nodes=nn, cb=cb),
